...
if (isset($_GET['delete_id'])) {
    ...
    $del_sql = "DELETE FROM company_types WHERE id = $delete_id";
    if (mysqli_query($conn, $del_sql)) {
        $message = "Company Type deleted successfully.";
    } else {
        $message = "Error deleting company type: " . mysqli_error($conn);
    }
}
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = mysqli_real_escape_string($conn, trim($_POST['name']));
    if ($name) {
        $sql = "INSERT INTO company_types (name) VALUES ('$name')";
        if (mysqli_query($conn, $sql)) {
            $message = "Company Type '$name' added successfully.";
        } else {
            $message = (mysqli_errno($conn) == 1062) ? "Company Type '$name' already exists." : "Error: " . mysqli_error($conn);
        }
    } else {
...
$types = [];
$result = mysqli_query($conn, "SELECT * FROM company_types ORDER BY name");
if ($result) {
...
<div class="max-w-xl mx-auto p-6 bg-white rounded shadow mt-8">
    ...
    <form method="post" class="flex gap-3 w-full mb-4">
        <input type="text" name="name" placeholder="Company Type name" required
    ...
    <div class="mb-4 flex flex-col sm:flex-row items-center gap-3">
        <input type="text" id="typeSearch" placeholder="Search company type..." class="flex-1 border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-400" />
    ...
    <h2 class="mt-8 mb-3 text-xl font-semibold border-b border-gray-200 pb-2">Existing Company Types
        ...
        <?php if (count($types) > 0): ?>
            <?php foreach ($types as $type): ?>
                <div class="type-card flex flex-col items-center border border-gray-100 rounded p-4 bg-gray-50">
                    ...
                    <a href="?delete_id=<?php echo $type['id']; ?>" onclick="return confirm('Delete this company type?');"
                        class="text-red-600 hover:text-red-800 text-sm font-semibold">Delete</a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <div class="italic text-gray-500 col-span-full">No company types added yet.</div>
        <?php endif; ?>
    </div>
</div>
<script>
