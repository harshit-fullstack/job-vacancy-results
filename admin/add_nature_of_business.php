...
if (isset($_GET['delete_id'])) {
    ...
    $del_sql = "DELETE FROM nature_of_business WHERE id = $delete_id";
    if (mysqli_query($conn, $del_sql)) {
        $message = "Nature of Business deleted successfully.";
    } else {
        $message = "Error deleting nature of business: " . mysqli_error($conn);
    }
}
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $name = mysqli_real_escape_string($conn, trim($_POST['name']));
    if ($name) {
        $sql = "INSERT INTO nature_of_business (name) VALUES ('$name')";
        if (mysqli_query($conn, $sql)) {
            $message = "Nature of Business '$name' added successfully.";
        } else {
            $message = (mysqli_errno($conn) == 1062) ? "Nature of Business '$name' already exists." : "Error: " . mysqli_error($conn);
        }
    } else {
...
$types = [];
$result = mysqli_query($conn, "SELECT * FROM nature_of_business ORDER BY name");
if ($result) {
...
<head>
<meta charset="UTF-8" />
<title>Add Nature of Business</title>
...
<div class="max-w-xl mx-auto p-6 bg-white rounded shadow mt-8">
    ...
    <form method="post" class="flex gap-3 w-full mb-4">
        <input type="text" name="name" placeholder="Nature of Business name" required
    ...
    <div class="mb-4 flex flex-col sm:flex-row items-center gap-3">
        <input type="text" id="typeSearch" placeholder="Search nature of business..." class="flex-1 border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-yellow-400" />
    ...
    <h2 class="mt-8 mb-3 text-xl font-semibold border-b border-gray-200 pb-2">Existing Nature of Business
    ...
    <div id="typesGrid" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <?php if (count($types) > 0): ?>
            <?php foreach ($types as $type): ?>
                <div class="type-card flex flex-col items-center border border-gray-100 rounded p-4 bg-gray-50">
                    <span class="font-semibold mb-2"><?php echo htmlspecialchars($type['name']); ?></span>
                    <a href="?delete_id=<?php echo $type['id']; ?>" onclick="return confirm('Delete this nature of business?');"
                        class="text-red-600 hover:text-red-800 text-sm font-semibold">Delete</a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
            <div class="italic text-gray-500 col-span-full">No nature of business added yet.</div>
        <?php endif; ?>
    </div>
</div>
<script>
